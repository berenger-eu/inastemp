...
public:
    ...
    inline InaVecAVX exp10() const {
#ifdef __INTEL_COMPILER
        return _mm256_exp10_ps(vec);
#else
        const __m256 COEFF_LOG210 = _mm256_set1_ps(float(InaFastExp::CoeffLog210()));
        const __m256 COEFF_A     = _mm256_set1_ps(float(InaFastExp::CoeffA32()));
        const __m256 COEFF_B     = _mm256_set1_ps(float(InaFastExp::CoeffB32()));
        const __m256 COEFF_P5_A  = _mm256_set1_ps(float(InaFastExp::GetCoefficient6_5()));
        const __m256 COEFF_P5_B  = _mm256_set1_ps(float(InaFastExp::GetCoefficient6_4()));
        ...
        __m256 x = _mm256_mul_ps(vec, COEFF_LOG210);
    ...
    inline static InaVecAVX<float> Fma(const InaVecAVX<float>& inValAdd, const InaVecAVX<float>& inValMul1, const InaVecAVX<float>& inValMul2){
        return _mm256_add_ps(inValAdd.vec, _mm256_mul_ps(inValMul1.vec,inValMul2.vec));
    }
};
